...
The painted boundary is now the first presented `pending` entry, and a settled match no longer takes the boundary. A layout route with no loader, such as one that is only lazy, now renders as soon as it settles, and the fallback stays on the match that is still loading.
