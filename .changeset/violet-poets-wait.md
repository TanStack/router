...
- `isModuleNotFoundError` now recognizes webpack/rspack `ChunkLoadError`s, so `lazyRouteComponent`'s automatic one-shot reload after a deploy also works on those bundlers, not just Vite/native ESM.
- Fuzzy (unmatched-URL) 404s now attribute the notFound to the deepest matched route with children that defines a `notFoundComponent`, so pathless layout `notFoundComponent`s render instead of `defaultNotFoundComponent` — consistent with how thrown `notFound()`s select their boundary. When no matched route defines one, the previous attribution is unchanged, and `notFoundMode: 'root'` still targets the root.
...
- Vue: `onResolved` and `onRendered` router events now carry real location-change info (`fromLocation` was previously always equal to `toLocation`, so `pathChanged`/`hrefChanged` were always `false` and cross-location scroll restoration never ran).
...
- Behavior change (Solid): `useMatch({ from })` no longer suppresses its missing-match invariant during a navigation whose destination includes `from` before that navigation's first commit — it throws (or returns `undefined` when `shouldThrow: false`) exactly as it does at idle, matching React.

**Removed / changed public API**

- `RouterState` no longer includes `loadedAt`, `isTransitioning`, `statusCode`, or `redirect`. `statusCode` and `redirect` are now server-only instance properties (`router.statusCode`, `router.redirect`) and are `undefined` on the client.
- Removed `RouterCore` members: `cancelMatch(id)`, `beforeLoad()`, `hasNotFoundMatch()`, `clearExpiredCache()` (expired cache entries are now dropped during commit-time reconciliation), `looseRoutesById` (use `routesById`), `getParsedLocationHref()`, and `viewTransitionPromise`.
- `getInitialRouterState` is no longer exported from `@tanstack/router-core`.
- `RouteMatch.fetchCount` has been removed.
- Client-side, a caught redirect's `headers` no longer include a `Location` header; response headers are server behavior, and `router.resolveRedirect` only sets `Location` on the server.
- `router.subscribe('onBeforeNavigate')` now also fires for redirect-replacement navigations (previously suppressed while a redirect was being applied).
- `router.startViewTransition(fn)` now returns `Promise<void>` (previously `void`).
