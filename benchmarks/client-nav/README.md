...
| `hydration`                              | Initial DOM hydration: execute the SSR payload, restore `beforeLoad` context and loader data, and hydrate 192 ordinary and eight hash-sensitive Links through their follow-up effects in Vue.                                                                                                                                        |
...
`scenarios/hydration/vue` provides an initial-hydration
...
```bash
CI=1 NX_DAEMON=false pnpm nx run @benchmarks/client-nav-hydration-vue:test:perf --outputStyle=stream --skipRemoteCache
CI=1 NX_DAEMON=false pnpm nx run @benchmarks/client-nav-hydration-vue:test:unit --outputStyle=stream --skipRemoteCache
CI=1 NX_DAEMON=false pnpm nx run @benchmarks/client-nav-hydration-vue:test:types:client --outputStyle=stream --skipRemoteCache
```
...
Vue bounds settlement to 100 turns. Ending at the hydration call or the
