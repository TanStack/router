    ...
    errorComponent: ({ error }) => {
      ...
      return (
        <div>
          ...
          <div data-testid="router-context">{context.value.routerValue}</div>
          <div data-testid="root-context">{context.value.rootValue}</div>
          <div data-testid="parent-context">{context.value.parentValue}</div>
          <div data-testid="child-context">
            {'childValue' in context.value
...
test('a same-id reload keeps the committed beforeLoad context visible until the next result', async () => {
  ...
  const rootRoute = createRootRoute({
    ...
    component: () => {
      ...
      return <div data-testid="locale">{context.value.locale ?? 'missing'}</div>
    },
  })
  const router = createRouter({
    routeTree: rootRoute,
