...
describe('redirect', () => {
  ...
  describe('SSR', () => {
    ...
    test('when `redirect` is thrown in `context`', async () => {
      ...
      const stateRedirect = router.state.redirect
      expect(stateRedirect).toBeDefined()
      expect(stateRedirect).toBeInstanceOf(Response)

      expect(stateRedirect!.options).toEqual({
        _fromLocation: expect.objectContaining({
          hash: '',
          href: '/',
          pathname: '/',
