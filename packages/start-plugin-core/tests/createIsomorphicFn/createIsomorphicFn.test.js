...
describe('createIsomorphicFn compiles correctly', async () => {
    const noImplWarning = 'createIsomorphicFn called without a client or server implementation!';
    ...
    describe.each(filenames)('should handle "%s"', async (filename) => {
        const file = await readFile(path.resolve(import.meta.dirname, `./test-files/${filename}`));
        const code = file.toString();
        test.each(['client', 'server'])(`should compile for ${filename} %s`, async (env) => {
            ...
            await expect(compiledResult.code).toMatchFileSnapshot(`./snapshots/${env}/${filename}`);
        });
    });
    ...
    test('should warn to console if no implementations provided', () => {
      ...
      const noImpl = createIsomorphicFn()`,
        ...
        expect(consoleSpy).toHaveBeenCalledWith(noImplWarning, 'This will result in a no-op function.', 'Variable name:', 'noImpl');
    });
});
