...
describe('pruneServerRoutePieces', () => {
  it('keeps a virtual loader route pending beneath a client-only root', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'tsr-virtual-loader-'))
    ...
    try {
      ...
      await fs.writeFile(
        path.join(routesDirectory, 'virtual.loader.ts'),
        "export const Route = createFileRoute('/virtual')({}); export const loader = () => 'client-loader-value'",
      )
      ...
      expect(code).not.toContain('virtual.loader')
      // Execute the generated tree with the router's real route constructors.
      const executable = code
        .replace(/^import[\s\S]*?from ['"][^'"]+['"];?\s*$/gm, '')
        .replace('export const routeTree', 'const routeTree')
      const routeTree = new Function(
        'rootRouteImport',
        'createFileRoute',
        `${executable}\nreturn routeTree`,
      )(new BaseRootRoute({ ssr: false }), () => () => {
        const route: AnyRoute = new BaseRoute()
        route.isRoot = false
        return route
      })
      const router = new RouterCore(
        {
          routeTree,
          isServer: true,
          history: createMemoryHistory({ initialEntries: ['/virtual'] }),
        },
        () => ({
      ...
      const matches = router.matchRoutes('/virtual')
      expect(matches.at(-1)?.routeId).toBe('/virtual')
      expect(matches.at(-1)?.status).toBe('pending')
    } finally {
  ...
  it('emits only route-piece imports available to server rendering', async () => {
    ...
    const routeFiles = {
      ...
      'none.lazy.tsx':
        "export const Route = createLazyFileRoute('/none')({ notFoundComponent: () => 'lazy-not-found' })",
      'data.tsx':
