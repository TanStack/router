...
function connect() {
  ...
  es.onmessage = (ev) => {
    ...
    if (type === 'error') {
      const message = typeof msg === 'object' && msg?.error ? String(msg.error) : 'Rebuild failed';
      let el = document.getElementById('tanstack-bun-error-overlay');
      if (!el) {
        el = document.createElement('pre');
        el.id = 'tanstack-bun-error-overlay';
        el.setAttribute('style', 'position:fixed;inset:0;z-index:2147483647;margin:0;padding:24px;overflow:auto;background:rgba(15,15,15,.94);color:#ffb4b4;font:14px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap');
        document.body.appendChild(el);
      }
      el.textContent = '[tanstack-start-bun] ' + message;
      return;
    }
    const overlay = document.getElementById('tanstack-bun-error-overlay');
