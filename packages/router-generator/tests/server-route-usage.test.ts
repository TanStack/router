...
describe('resolveServerSsr', () => {
  ...
  it('publishes inherited usage for route files across unchanged runs', async () => {
    ...
    try {
      ...
      expect(usage('__root.tsx')).toBeUndefined()
  ...
  it.each([false, 'data-only'] as const)(
    'preserves root shell code while inheriting %s for children',
    async (ssr) => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), 'tsr-root-shell-'))
      const routesDirectory = path.join(root, 'routes')
      await fs.mkdir(routesDirectory)
      try {
        await fs.writeFile(
          path.join(routesDirectory, '__root.tsx'),
          `export const Route = createRootRoute({ ssr: ${JSON.stringify(ssr)} })`,
        )
        await fs.writeFile(
          path.join(routesDirectory, 'child.tsx'),
        ...
        await generator.run()
        const routes = generator.getRoutesByFileMap()
        expect(
          routes.get(
            path.join(routesDirectory, '__root.tsx').replaceAll('\\', '/'),
          )?.serverSsr,
        ).toBeUndefined()
        expect(
          routes.get(
            path.join(routesDirectory, 'child.tsx').replaceAll('\\', '/'),
          )?.serverSsr,
        ).toBe(ssr)
      } finally {
        await fs.rm(root, { recursive: true, force: true })
