...
function setupConfig(
  ...
  const { generatedRouteTree = '/routeTree.nonnested.gen.ts', ...rest } =
    inlineConfig
  ...
  switch (folderName) {
    ...
    case 'types-disabled':
      ...
      config.generatedRouteTree =
        makeFolderDir(folderName) + '/routeTree.nonnested.gen.js'
      break
...
describe('generator works', async () => {
  ...
  it.each(folderNames.map((folder) => [folder]))(
    ...
    async (folderName) => {
      ...
      if (error) {
        ...
        let snapshotPath = `routeTree.nonnested.snapshot.${config.disableTypes ? 'js' : 'ts'}`
  ...
  it('should create directory for routeTree if it does not exist', async () => {
    ...
    const folderRoot = makeFolderDir(folderName)
    await fs.rm(join(folderRoot, 'generated'), { recursive: true, force: true })
    ...
    if (error) {
      ...
      await expect(generatedRouteTree).toMatchFileSnapshot(
        join(
          'generator',
          folderName,
          `routeTree.nonnested.generated.${config.disableTypes ? 'js' : 'ts'}`,
        ),
      )
