...
${acc.routeTree.map((child) => `${child.variableName}Route: typeof ${getResolvedRouteNodeVariableName(child)}`).join(',')}
  ...
  ${acc.routeTree
    ...
    // When isolatedDeclarations is enabled, remove "// @ts-nocheck" from the
    // header — the feature only works if TypeScript actually type-checks the
    // file, so keeping the opt-out comment would silently defeat its purpose.
    const header = config.isolatedDeclarations
      ? config.routeTreeFileHeader.filter(
          (line) => line.trim() !== '// @ts-nocheck',
        )
