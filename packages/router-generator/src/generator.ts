...
function assignServerSsr(
  ...
  // Shell requests always execute the root, even when its SSR option is false.
  rootRouteNode.serverSsr = undefined

  const serverSsrByRoutePath = new Map<string, StaticSsrOption | undefined>()
  if (rootRouteNode.routePath) {
    serverSsrByRoutePath.set(rootRouteNode.routePath, undefined)
  }

  acc.routeNodes.forEach((node) => {
    const parentSsr =
      node.parent && node.parent !== rootRouteNode
        ? node.parent.serverSsr
        : rootSsr
...
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.`,
  ...
  private async handleRootNode(node: RouteNode) {
    ...
    node.staticSsr =
      result.status === 'fresh'
        ? result.cacheEntry.node.staticSsr
        : getStaticSsrOption(updatedCacheEntry.fileContent, node.fullPath)
