...
export async function ensureBoilerplate(node: RouteNode, code: string) {
                ...
                if (!foundImport) {
                  programPath.node.body.unshift(
                    babel.template.statement(
                      `import { RouteRoute } from '@tanstack/react-router'`,
                    )(),
                  )
                }
              } else {
  ...
  // const separator = node.isRoot ? 'new RootRoute(' : 'new Route('
          ...
          enter(programPath, state) {
            ...
            programPath.traverse({
              ...
              Identifier(iPath) {
                ...
                if (node.isRoot) {
                  if (iPath.node.name === 'RootRoute') {
                    if (t.isNewExpression(iPath.parentPath.node)) {
                      if (
                      ...
                      if (
                        ...
                        if (clientOnly) {
                          return t.objectProperty(
                            t.identifier(key),
                            babel.template.expression(`
                                lazyRouteComponent(() => import('./${path.relative(
                                  node.genDir,
                                  node.genPathNoExt,
                                )}-${key}'))`)() as any,
                          )
...
function getRouteConfigGenerateOptions(path: any): t.ObjectExpression | void {
  ...
  if (t.isNewExpression(path.parentPath?.node)) {
