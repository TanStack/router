...
import { X_TSS_SERIALIZED } from '@tanstack/start-client-core'
import { RawStream } from '@tanstack/router-core'
import { defaultSerovalPlugins } from '@tanstack/router-core/ssr/server'
...
vi.mock('@tanstack/start-client-core', async (importOriginal) => {
  ...
  return {
    ...actual,
    getSerovalPlugins: () => defaultSerovalPlugins,
  }
})
...
test('frames every record from a synchronously replayed Seroval stream', async () => {
  ...
  for (let index = 0; index < frames.length; index++) {
    const value = fromCrossJSON(
      JSON.parse(new TextDecoder().decode(frames[index]!.payload)),
      { refs, plugins: defaultSerovalPlugins },
    )
    if (index === 0) {
      result = value
    }
...
test('rejects a non-ASCII JSON record larger than the wire limit', async () => {
  ...
  await expect(response.body!.getReader().read()).rejects.toThrow(
    'pending output limit',
  )
})

test('cancels framed serialization when Response construction rejects it', async () => {
  const cancel = vi.fn()
  const source = new ReadableStream<Uint8Array>({ cancel })
