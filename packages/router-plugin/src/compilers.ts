...
import { parseAst, type ParseAstOpts } from './ast'
...
export function getCodeSplitReferenceRoute(opts: ParseAstOpts) {
  ...
  if (!ast) {
    throw new Error(
      `Failed to compile ast for a Reference Route for the file: ${opts.filename}`,
    )
  }
...
export function getCodeSplitVirtualRoute(opts: ParseAstOpts) {
  let ast = parseAst(opts)

  if (!ast) {
    throw new Error(
      `Failed to compile ast for a Split Route for the file: ${opts.filename}`,
    )
  }
  ...
  traverse(ast, {
    Program: {
      enter(programPath, programState) {
        let state = (programState || {}) as unknown as State
