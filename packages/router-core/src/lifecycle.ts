...
type ResolveDehydrateFromOption<TOption> = TOption extends {
  ...
  ? Extract<TDehydrate, AnyLifecycleFunction>
  : never

type ResolveHydrateFromOption<TOption> = TOption extends {
  hydrate?: infer THydrate
}
  ? Extract<THydrate, AnyLifecycleFunction>
  : never

type ResolveRevalidateFromOption<TOption> = TOption extends {
  revalidate?: infer TRevalidate
}
  ? Extract<TRevalidate, AnyLifecycleFunction>
  : never

type FallbackLifecycleFunction<TFn> = [TFn] extends [never]
  ? AnyLifecycleFunction
  : TFn
...
): FallbackLifecycleFunction<ResolveHandlerFromOption<TOption>> | undefined {
  if (option === undefined) return undefined
  if (typeof option === 'function') {
    return option as FallbackLifecycleFunction<
      ResolveHandlerFromOption<TOption>
    >
  }
  return option.handler as FallbackLifecycleFunction<
    ResolveHandlerFromOption<TOption>
  >
}
...
export function getDehydrateFn<TOption extends AnyLifecycleOption>(
  option: TOption | undefined,
): FallbackLifecycleFunction<ResolveDehydrateFromOption<TOption>> | undefined {
  if (!option || typeof option === 'function') return undefined
  const d = option.dehydrate
  return (typeof d === 'function' ? d : undefined) as
    | FallbackLifecycleFunction<ResolveDehydrateFromOption<TOption>>
    | undefined
}

export function getHydrateFn<TOption extends AnyLifecycleOption>(
  option: TOption | undefined,
): FallbackLifecycleFunction<ResolveHydrateFromOption<TOption>> | undefined {
  if (!option || typeof option === 'function') return undefined
  const h = option.hydrate
  return (typeof h === 'function' ? h : undefined) as
    | FallbackLifecycleFunction<ResolveHydrateFromOption<TOption>>
    | undefined
}

export function getRevalidateFn<TOption extends AnyLifecycleOption>(
  option: TOption | undefined,
): FallbackLifecycleFunction<ResolveRevalidateFromOption<TOption>> | undefined {
  if (!option || typeof option === 'function') return undefined
  const r = option.revalidate
  return (typeof r === 'function' ? r : undefined) as
    | FallbackLifecycleFunction<ResolveRevalidateFromOption<TOption>>
    | undefined
}
