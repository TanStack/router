...
describe('client lane synchronous work', async () => {
  ...
  const resolvedValue = Promise.resolve(synchronousValue)
  const resolvedWaits = async () => {
    for (let iteration = 0; iteration < 80; iteration++) {
      await waitFor(resolvedValue, signal)
    }
  }
  ...
  bench('80 waits for an already-resolved Promise', resolvedWaits, benchOptions)
