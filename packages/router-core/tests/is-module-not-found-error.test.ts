...
describe('isModuleNotFoundError', () => {
  ...
  it('detects ChunkLoadError independently of its message', () => {
    const error = new Error('The requested module is unavailable')
    error.name = 'ChunkLoadError'
    expect(isModuleNotFoundError(error)).toBe(true)
  })

  it.each([
    'Loading chunks from local storage failed',
    'Loading chunk metadata is unavailable',
  ])('does not classify unrelated errors: %s', (message) => {
    expect(isModuleNotFoundError(new Error(message))).toBe(false)
  })
