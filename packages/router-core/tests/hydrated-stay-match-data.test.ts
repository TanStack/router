...
describe('hydrated stay match data preservation', () => {
  ...
  it('keeps server loader data on the root stay match across a client navigation', async () => {
    ...
    // Let the follow-up client load (for the ssr:false hole) settle.
    await vi.waitFor(() => {
      expect(
        router.state.matches.find((m) => m.routeId === aRoute.id)?.status,
      ).toBe('success')
    })
    ...
    await vi.waitFor(() => {
      expect(
        router.state.matches.find((m) => m.routeId === bRoute.id)?.status,
      ).toBe('success')
    })
