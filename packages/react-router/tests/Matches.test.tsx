...
describe('useMatchRoute with React Compiler', () => {
  afterEach(() => cleanup())

  test('should update match result after navigation (React Compiler compatibility)', async () => {
    // This test demonstrates the stale closure issue with React Compiler
    // When React Compiler is enabled, useMatchRoute's returned function
    ...
    function TestComponent() {
      ...
      // Check if we're currently at /about
      const isAbout = matchRoute({ to: '/about' })

      return (
        <div>
          <div data-testid="is-home">{isHome ? 'true' : 'false'}</div>
          <div data-testid="is-about">{isAbout ? 'true' : 'false'}</div>
    ...
    // Initially at /home, should match home but not about
    expect(await screen.findByTestId('is-home')).toHaveTextContent('true')
    expect(await screen.findByTestId('is-about')).toHaveTextContent('false')

    // Navigate to /about
    const navLink = await screen.findByTestId('nav-to-about')
    await act(async () => {
      fireEvent.click(navLink)
    })

    // After navigation to /about:
    // - isHome should be false (we're no longer at /home)
    // - isAbout should be true (we're now at /about)
    //
    // WITHOUT FIX (with React Compiler):
    //   The matchRoute function captures stale router state from /home,
    //   so it still thinks we're at /home even after navigation.
    //   This causes isHome to remain true and isAbout to remain false.
    //
    // WITH FIX:
    //   The matchRoute function properly reads current router state,
    //   so it correctly identifies we're now at /about.
    expect(await screen.findByTestId('is-home')).toHaveTextContent('false')
    expect(await screen.findByTestId('is-about')).toHaveTextContent('true')
  })
})
