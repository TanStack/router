...
describe('lifecycle method semantics', () => {
  ...
  describe('context caching behavior', () => {
    test('context does NOT re-run on search param changes (match ID unchanged)', async () => {
      ...
      const indexRoute = createRoute({
        ...
        component: () => {
          ...
          return (
            <div>
              <h1 data-testid="index-heading">Index page</h1>
              ...
              <button
                data-testid="change-tab-again"
                onClick={() => navigate({ search: { tab: 'profile' } })}
              >
                change-tab-again
              </button>
            </div>
      ...
      await screen.findByTestId('index-heading')
      // context runs once on initial match creation
      expect(mockContext).toHaveBeenCalledTimes(1)
      mockContext.mockClear()

      // Change search params — match ID doesn't change
      fireEvent.click(await screen.findByTestId('change-tab'))
      await waitFor(() => {
      ...
      fireEvent.click(await screen.findByTestId('change-tab-again'))
      await waitFor(() => {
        expect(screen.getByTestId('search').textContent).toBe(
          JSON.stringify({ tab: 'profile' }),
        )
      })
      expect(mockContext).not.toHaveBeenCalled()
    })

    test('context does NOT re-run on router.invalidate()', async () => {
      const mockContext = vi.fn()
      ...
      // Invalidate should re-run loader but NOT context
      await act(async () => {
        await router.invalidate()
      })
    ...
    test('context does NOT re-run when navigating away and back (match cached)', async () => {
      const mockContext = vi.fn()
      ...
      fireEvent.click(await screen.findByTestId('go-to-other'))
      await screen.findByTestId('other-heading')

      // Navigate back — match is found in cache, context should NOT fire again
      fireEvent.click(await screen.findByTestId('go-to-index'))
      await screen.findByTestId('index-heading')
      expect(mockContext).not.toHaveBeenCalled()
    })

    test('context re-runs when navigating away and back after match is GC-ed', async () => {
      const mockContext = vi.fn()
      ...
      // Navigate away — match goes to cachedMatches then is immediately GC-ed
      fireEvent.click(await screen.findByTestId('go-to-other'))
      await screen.findByTestId('other-heading')

      // Navigate back — match was evicted from cache, so it is re-created
      fireEvent.click(await screen.findByTestId('go-to-index'))
      await screen.findByTestId('index-heading')
      expect(mockContext).toHaveBeenCalledTimes(1)
    })
  ...
  describe('context with revalidate caching behavior', () => {
    test('context with revalidate: true re-runs on router.invalidate()', async () => {
      const mockContext = vi.fn()
      ...
      // Invalidate should re-run context with revalidate: true
      await act(async () => {
        await router.invalidate()
      })
    ...
    test('context with revalidate does NOT re-run on search param changes that do not affect loaderDeps', async () => {
      const mockContext = vi.fn()
        ...
        component: () => {
          ...
          return (
            <div>
              ...
              <button
                data-testid="change-foo"
                onClick={() =>
                  navigate({
                    search: (s: any) => ({ ...s, foo: 'foo-1' }),
                  })
                }
              >
                change-foo
      ...
      // Change bar (not in loaderDeps) — context should NOT re-run
      fireEvent.click(await screen.findByTestId('change-bar'))
      await waitFor(() => {
        expect(screen.getByTestId('search').textContent).toBe(
          JSON.stringify({ bar: 'bar-1' }),
        )
      })
      expect(mockContext).not.toHaveBeenCalled()

      // Change foo (in loaderDeps) — context with revalidate SHOULD re-run
      fireEvent.click(await screen.findByTestId('change-foo'))
      await waitFor(() => {
        expect(screen.getByTestId('search').textContent).toBe(
          JSON.stringify({ bar: 'bar-1', foo: 'foo-1' }),
        )
      })
      expect(mockContext).toHaveBeenCalledTimes(1)
    })

    test('context with revalidate does NOT re-run when navigating away and back (match cached)', async () => {
      const mockContext = vi.fn()
      ...
      // Navigate back — match is found in cache, needsContext already consumed
      fireEvent.click(await screen.findByTestId('go-to-index'))
      await screen.findByTestId('index-heading')
      expect(mockContext).not.toHaveBeenCalled()
    })

    test('context with revalidate does NOT re-run when navigating away and back (cached by loader + context)', async () => {
      const mockContext = vi.fn()
      const mockLoader = vi.fn()
      ...
      // Navigate back — match is found in cache, needsContext consumed.
      // Loader re-runs (staleTime=0, stale) but context does not.
      fireEvent.click(await screen.findByTestId('go-to-index'))
      await screen.findByTestId('index-heading')
      expect(mockContext).not.toHaveBeenCalled()
      // Loader still re-runs based on its own staleTime logic
      expect(mockLoader).toHaveBeenCalledTimes(1)
    })
  })

  describe('context flow through all lifecycle methods', () => {
    test('context accumulates through context -> beforeLoad -> loader on a single route', async () => {
      const mockLoader = vi.fn()
      ...
      const indexRoute = createRoute({
        ...
        path: '/',
        context: () => {
          return { fromContext: 'context-value' }
        },
        beforeLoad: ({ context }) => {
          // Should see context return
          expect(context).toEqual(
            expect.objectContaining({ fromContext: 'context-value' }),
          )
          return { fromBeforeLoad: 'beforeload-value' }
        },
        loader: ({ context }) => {
          // Should see context + beforeLoad context
          mockLoader(context)
        },
        component: () => {
          const context = indexRoute.useRouteContext()
      ...
      expect(context).toEqual({
        ...
        fromBeforeLoad: 'beforeload-value',
      })
    ...
    test('context accumulates with router-level context through all lifecycle methods', async () => {
      const mockLoader = vi.fn()
      ...
      const indexRoute = createRoute({
        ...
        path: '/',
        context: ({ context }) => {
          return { ...context, fromContext: true }
        },
        beforeLoad: ({ context }) => {
          return { ...context, fromBeforeLoad: true }
        },
        loader: ({ context }) => {
          mockLoader(context)
        },
  ...
  describe('parent-child context inheritance (3+ levels)', () => {
    test('child routes inherit accumulated context from all ancestors', async () => {
      const mockGrandchildLoader = vi.fn()

      const rootRoute = createRootRoute({
        context: () => ({ rootContext: 'root' }),
        beforeLoad: () => ({ rootBeforeLoad: 'root-bl' }),
        component: () => (
          <div>
            Root <Outlet />
          </div>
      ...
      const parentRoute = createRoute({
        ...
        path: '/parent',
        context: ({ context }) => {
          // Should see root's full context
          expect(context).toEqual(
            expect.objectContaining({
              rootContext: 'root',
              rootBeforeLoad: 'root-bl',
            }),
          )
          return { parentContext: 'parent' }
        },
        beforeLoad: () => ({ parentBeforeLoad: 'parent-bl' }),
        component: () => (
          <div>
            Parent <Outlet />
          </div>
      ...
      const childRoute = createRoute({
        ...
        path: '/child',
        context: ({ context }) => {
          // Should see root + parent full context
          expect(context).toEqual(
            expect.objectContaining({
              rootContext: 'root',
              rootBeforeLoad: 'root-bl',
              parentContext: 'parent',
              parentBeforeLoad: 'parent-bl',
            }),
          )
          return { childContext: 'child' }
        },
        beforeLoad: () => ({ childBeforeLoad: 'child-bl' }),
        loader: ({ context }) => {
          mockGrandchildLoader(context)
        },
        component: () => {
          const context = childRoute.useRouteContext()
          return (
            <div data-testid="child-context">{JSON.stringify(context)}</div>
          )
        },
      })
      ...
      const contextEl = await screen.findByTestId('child-context')
      const context = JSON.parse(contextEl.textContent)
      ...
      expect(context).toEqual(
        expect.objectContaining({
          ...
          parentBeforeLoad: 'parent-bl',
          childContext: 'child',
          childBeforeLoad: 'child-bl',
        }),
      )

      expect(mockGrandchildLoader).toHaveBeenCalledWith(
        expect.objectContaining({
          rootContext: 'root',
          rootBeforeLoad: 'root-bl',
          parentContext: 'parent',
  ...
  describe('async support', () => {
    test('context supports async execution', async () => {
      ...
      const indexRoute = createRoute({
        ...
        context: async () => {
          ...
          return { fromAsyncContext: 'async-value' }
        },
        beforeLoad: ({ context }) => {
          executionOrder.push('beforeLoad')
          // context should have completed before beforeLoad runs
          expect(context).toEqual(
            expect.objectContaining({ fromAsyncContext: 'async-value' }),
          )
        },
        component: () => {
          const context = indexRoute.useRouteContext()
  ...
  describe('combined invalidation behavior', () => {
    test('router.invalidate() re-runs context with revalidate and loader but NOT plain context', async () => {
      let contextCount = 0
      let loaderCount = 0
      ...
      const indexRoute = createRoute({
        ...
        context: {
          handler: () => {
            contextCount++
            return { contextRun: contextCount }
          },
          revalidate: true,
        },
        loader: () => {
      ...
      expect(contextCount).toBe(2) // Re-run (revalidate: true)
      expect(loaderCount).toBe(2) // Re-run
      ...
      expect(contextCount).toBe(3) // Re-run again
      expect(loaderCount).toBe(3) // Re-run again
    })
  })
  ...
  describe('context edge cases', () => {
    ...
    test('context returning undefined does not clobber parent context', async () => {
      ...
      const indexRoute = createRoute({
        ...
        path: '/',
        context: () => {
          // returning undefined — should not overwrite context
          return undefined
        },
        beforeLoad: ({ context }) => {
          // rootValue should still be visible
    ...
    test('context receives correct params', async () => {
      ...
      const userRoute = createRoute({
        ...
        component: () => {
          ...
          return <div data-testid="user-context">{JSON.stringify(context)}</div>
        },
      })
  ...
  describe('context with revalidate edge cases', () => {
    test('context with revalidate re-runs when loaderDeps change (new matchId)', async () => {
      ...
      // Change to page=1 — new loaderDeps → new matchId → context re-runs
      ...
      // Change to page=2 — new loaderDeps → new matchId → context re-runs
    ...
    test('context with revalidate re-runs after GC (navigating away, match evicted, navigating back)', async () => {
      const mockContext = vi.fn()
      ...
      // Navigate back — fresh match, context should re-run
      fireEvent.click(await screen.findByTestId('go-index'))
      await screen.findByTestId('index-page')
      expect(mockContext).toHaveBeenCalledTimes(1)
    })
  ...
  describe('parent-child selective GC', () => {
    test('when child match is GC-ed but parent is not, only child context re-runs', async () => {
      ...
      // Navigate to parent index — child match goes to cache and gets GC-ed (gcTime: 0)
      // Parent match stays active
      fireEvent.click(await screen.findByTestId('go-parent-only'))
      await screen.findByTestId('parent-index-page')

      // Parent should NOT re-run (still active)
      expect(parentContext).not.toHaveBeenCalled()

      // Navigate back to child — child match was GC-ed, fresh creation
      fireEvent.click(await screen.findByTestId('go-child'))
      await screen.findByTestId('child-page')

      // Parent still should NOT re-run (stayed matched the whole time)
      expect(parentContext).not.toHaveBeenCalled()

      // Child should re-run (fresh match after GC)
      expect(childContext).toHaveBeenCalledTimes(1)
    })
  })

  describe('context-with-revalidate-only routes (no loader)', () => {
    test('route with only context (revalidate) and no loader works correctly', async () => {
      const mockContext = vi.fn()
      ...
      const indexRoute = createRoute({
        getParentRoute: () => rootRoute,
        path: '/',
        // No loader — only context with revalidate
        context: {
          handler: () => {
            mockContext()
      ...
      // Navigate back — match is cached, context should NOT re-run
      fireEvent.click(await screen.findByTestId('go-index'))
  ...
  describe('context updates on invalidation', () => {
    test('context with revalidate updates after each invalidation', async () => {
      let counter = 0
    ...
    test('object form with dehydrate flag still runs handler on client navigation', async () => {
      ...
      const aboutRoute = createRoute({
        ...
        context: {
          ...
          dehydrate: true, // dehydrate flag has no effect on SPA navigation
        },
        component: () => {
          const context = aboutRoute.useRouteContext()
          return <div data-testid="context">{JSON.stringify(context)}</div>
