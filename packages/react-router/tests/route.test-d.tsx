...
test('object form context revalidate infers prev from handler', () => {
  const rootRoute = createRootRoute()
...
test('object form context revalidate must return handler data', () => {
  const rootRoute = createRootRoute()
  ...
  createRoute({
    ...
    context: {
      ...
      // @ts-expect-error revalidate must return the same data shape as handler
      revalidate: ({ prev }) => ({
        value: (prev?.value ?? 0) + 1,
      }),
    },
...
test('object form dehydrate and hydrate infer data across lifecycle methods', () => {
  const rootRoute = createRootRoute()
    ...
    context: {
      ...
      hydrate: ({ data }) => {
        ...
        return {
          ...
          format: (value: string) => `${data.label}:${value}`,
        }
      },
    },
    beforeLoad: {
    ...
    loader: {
      ...
      hydrate: ({ data }) => {
        ...
        return {
          ...
          scores: data.scores,
          computeAvg: () =>
            data.scores.reduce((total, score) => total + score, 0) /
            data.scores.length,
        }
      },
    },
...
test('object form dehydrate return must be serializable', () => {
  const rootRoute = createRootRoute()

  createRoute({
    getParentRoute: () => rootRoute,
    path: 'test',
    // @ts-expect-error dehydrate wire data must be serializable
    context: {
      handler: () => ({ label: 'ctx' }),
      dehydrate: ({ data }) => ({
        label: data.label,
