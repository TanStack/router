...
- Check test discovery and TypeScript/ESLint includes for newly added files. A passing source-only compiler matrix does not typecheck tests outside its includes; use the applicable test configuration and inspect executed cases. Report missing coverage explicitly and keep existing compiler/lint rules intact.
