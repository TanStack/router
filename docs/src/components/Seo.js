...
export const Seo = withRouter(
  ({ title, description, image = ogImage, router, children }) => (
    <Head>
      ...
      {title != undefined && (
        <title key="title">{title} | React Location | TanStack</title>
      )}
      ...
      <meta
        property="og:url"
        key="og:url"
        content={`https://react-location.tanstack.com${router.pathname}`}
      />
      {title != undefined && (
      ...
      {image != undefined && (
        <meta
          property="og:image"
          key="og:image"
          content={`https://react-location.tanstack.com${image}`}
        />
      )}
