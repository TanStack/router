...
| element        |          | `React.ReactNode | ({ params: Params }) => PromiseLike<React.ReactNode>` | The content to be rendered when the route is matched                                                                                                                |
| errorElement   |          | `React.ReactNode`                                                        | The content to be rendered when `loader` encounters an error                                                                                                        |
| pendingElement |          | `React.ReactNode`                                                        | The content to be rendered when the duration of `loader` execution surpasses the `pendingMs` duration                                                               |
...
| import         |          | `({ params: Params }) => PromiseLike<Omit<Route, 'path' | 'import'>>`    | An asyncronous function that resolves all of the above route information (everything but the `path` and `import` properties, of course). Useful for code-splitting! |
