...
test('accounts isolate drafts and mutations while publishing is explicit', async ({
  ...
  try {
    ...
    await page
      ...
      .fill('Do not insert this note')
    await page
      .getByLabel('Category', { exact: true })
      .fill(`${category}-rollback`)
    await page.getByRole('button', { name: 'Create note', exact: true }).click()
    await expect(page.getByRole('alert')).toHaveText(
      'That slug is already used. Choose another one.',
    )
    expect(
      await db.category.count({ where: { name: `${category}-rollback` } }),
    ).toBe(0)
    await page.getByLabel('Slug', { exact: true }).fill(`invalid-${id}`)
    await page.getByLabel('Title', { exact: true }).fill('   ')
    await page.getByRole('button', { name: 'Create note', exact: true }).click()
    await expect(page.getByRole('alert')).toHaveText(
      'Could not save. Check your input and try again.',
    )
    expect(await db.note.count({ where: { slug: `invalid-${id}` } })).toBe(0)
    ...
    await db.category.deleteMany({
      where: { name: { in: [category, `${category}-rollback`] } },
    })
  }
})
