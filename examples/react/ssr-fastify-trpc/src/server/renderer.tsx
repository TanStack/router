...
const renderer: FastifyViteRendererOptions = {
	...
	// @ts-expect-error
	createRenderFunction({ createApp }: { createApp: typeof ClientModule['createApp'] }) {
		...
		return async function (this: FastifyReply) {
			...
			router.update({
				...
				history: createMemoryHistory({ initialEntries: [this.request.originalUrl] }),
			});
			await router.load();
			this.statusCode = router.hasNotFoundMatch()
				? 404
				: 200;
	...
	createRoute(
		...
		scope.route({
			...
			method: 'GET',
			handler: handler as (request: FastifyRequest, reply: FastifyReply) => Promise<void>,
			errorHandler,
		});
	},
	createRouteHandler(
	...
	async prepareClient(
		...
		return Object.assign(
			...
			clientModule,
			{
				 ...
				 */
				routes: Object
					.entries(clientModule.createApp().routesById)
					.reduce(
						(pv, [key, { fullPath }]) => key === rootRouteId
							? pv
							: (() => {
