...
test('TransitionSuspense allows regular createResource to work with transitions', async ({
  page,
}) => {
  // Navigate to the transition-simple test route
  await page.goto('/transition-simple')
  ...
  // This proves TransitionSuspense works with regular createResource!
  const hasLoadingText = bodyTexts.some((text) => text.includes('Loading...'))

  if (hasLoadingText) {
    throw new Error(
      'FAILED: "Loading..." appeared during navigation. ' +
        'TransitionSuspense should keep old values visible with regular createResource.',
    )
  }
})
