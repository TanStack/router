...
const failingChildOnStayBlock =
  "  onStay: () => {\n    const match = (window as any).__TSR_ROUTER__.stores.matches\n      .get()\n      .find((candidate: any) => candidate.routeId === '/child')\n    ;(window as any).__TSR_HMR_FAILED_SIGNAL__ = match.abortController.signal\n    ;(window as any).__TSR_HMR_FAILURE_RAN__ = true\n    throw new Error('intentional HMR lifecycle failure')\n  },\n"
...
test.describe('react-start hmr', () => {
  ...
  test('updates child route loader data without rebuilding the route tree', async ({
    ...
    const preloadedRouteId = await page.evaluate(async () => {
      const matches = await (window as any).__TSR_ROUTER__.preloadRoute({
        to: '/inputs',
      })
      return matches?.at(-1)?.routeId
    })
    expect(preloadedRouteId).toBe('/inputs')
  })
  ...
  test('rolls back a failed route refresh and accepts the next HMR update', async ({
    ...
    await page.evaluate(() => {
      const match = (window as any).__TSR_ROUTER__.stores.matches
        .get()
        .find((candidate: any) => candidate.routeId === '/child')
      ;(window as any).__TSR_HMR_COMMITTED_SIGNAL__ =
    ...
    expect(
      await page.evaluate(() => ({
        committed: (window as any).__TSR_HMR_COMMITTED_SIGNAL__.aborted,
        failed: (window as any).__TSR_HMR_FAILED_SIGNAL__.aborted,
      })),
    ).toEqual({ committed: false, failed: true })
    ...
    expect(
      await page.evaluate(() => {
        const router = (window as any).__TSR_ROUTER__
        const current = router.stores.matches
          .get()
          .find((candidate: any) => candidate.routeId === '/child')
        return {
          previous: (window as any).__TSR_HMR_COMMITTED_SIGNAL__.aborted,
          current: current.abortController.signal.aborted,
        }
      }),
    ).toEqual({ previous: true, current: false })
  })
