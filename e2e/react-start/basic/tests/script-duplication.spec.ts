...
test.describe('Script Duplication Prevention', () => {
  ...
  test('should not create duplicate scripts during client-side navigation', async ({ page }) => {
  ...
  test('should not create duplicate scripts with multiple navigation cycles', async ({ page }) => {
    // Start from home page
    await page.goto('/')
  ...
  test('should not create duplicate inline scripts', async ({ page }) => {
    ...
    await expect(page.getByTestId('inline-scripts-test-heading')).toBeInViewport()
    
    // Count specific inline scripts
    const script1Count = await page.evaluate(() => {
      const scripts = Array.from(document.querySelectorAll('script:not([src])'))
      return scripts.filter(script => 
        script.textContent && script.textContent.includes('window.INLINE_SCRIPT_1 = true')
      ).length
    })
    ...
    const script2Count = await page.evaluate(() => {
      const scripts = Array.from(document.querySelectorAll('script:not([src])'))
      return scripts.filter(script => 
        script.textContent && script.textContent.includes('window.INLINE_SCRIPT_2 = "test"')
      ).length
    })
  ...
  test('should not create duplicate inline scripts during client-side navigation', async ({ page }) => {
    // Start from home page
    await page.goto('/')
