...
const clientSourceClassificationCases = [
  ['known-leaf', 'middleware', 'leaf'],
  ['known-parent', 'middleware', 'nested-parent'],
  ['known-root', 'middleware', 'group-root'],
  ['known-trailing', 'middleware', 'trailing-known'],
  ['known-handler', 'handler', 'handler-known'],
  ['unknown-trailing', 'unknown', 'trailing-unknown'],
  ['unknown-handler', 'unknown', 'handler-unknown'],
] as const

async function expectClientSourceClassificationResults(page: Page) {
  for (const [scenario, source, tag] of clientSourceClassificationCases) {
    await expect(page.getByTestId(`loader-${scenario}`)).toContainText(
      `"source": "${source}"`,
    )
    await expect(page.getByTestId(`loader-${scenario}`)).toContainText(
      `"tag": "${tag}"`,
...
test.describe('client middleware next() source classification', () => {
  test('classifies known middleware and handler during SSR', async ({
    page,
  }) => {
    await page.goto('/middleware/client-source-classification')
    await page.waitForLoadState('networkidle')

    await expectClientSourceClassificationResults(page)
  })

  test('classifies known middleware and handler on client-side navigation', async ({
    page,
  }) => {
    await page.goto('/middleware')
    await page.waitForLoadState('networkidle')

    await page.getByTestId('client-source-classification-link').click()
    await expect(
      page.getByTestId('client-source-classification-route'),
    ).toBeVisible()
