...
test.describe('Deferred data streaming', () => {
  test('finishes the router stream after renderer output', async ({
    baseURL,
  }) => {
    await expectDeferredRouteResponseOrder(baseURL)
  })

  test('renders immediate and deferred server data', async ({ page }) => {
    await page.goto('/deferred')

    // Immediate loader data is rendered.
  ...
  test('all data sources are server', async ({ page }) => {
    await page.goto('/deferred')
