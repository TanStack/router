...
test.describe('Client-side navigation between all routes', () => {
  ...
  test('home -> stream -> home works (no stream locking error)', async ({
    ...
    // Navigate back without waiting for stream completion.
    await page
      .getByRole('navigation')
      .getByRole('link', { name: 'Home' })
      .click()
  ...
  test('navigation without waiting for route data works', async ({ page }) => {
    // Console errors are monitored by the fixture automatically

    // Change routes as soon as each URL updates, without waiting for route data.
    await page
      .getByRole('navigation')
      .getByRole('link', { name: 'Sync Only' })
      .click()
